...
\end{itemize}
\vspace{6pt}
{\color{midGray}\footnotesize Even the benchmark full-core accident peak (about 1118\,\textcelsius) stays below the operating maximum.}
\end{column}
...
{\color{midGray}\footnotesize Geometry matches the benchmark (porosity 0.186, 79.3\,cm element), cross-checked against NEA/NSC/R(2017)4.}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{Action}
\eyebrow{Inputs}
{\color{midGray}\footnotesize All values trace to the OECD/NEA MHTGR-350 benchmark, report NEA/NSC/R(2017)4.}
\vspace{8pt}
...
{\small The peak-power column (\hot{$\times$1.85}) is the bounding safety number: within 1\% of the benchmark core max (1282\,\textcelsius), and above the full-core DCC peak (1118\,\textcelsius). The average column (\key{793\,\textcelsius}) validates the benchmark outlet. Both cool from their steady peak.}
\end{column}
\end{columns}
\end{frame}
...
{\footnotesize Converged near second order (GCI about 1\%). Coolant outlet \hot{688\,\textcelsius} against the benchmark \key{687\,\textcelsius}, a 0.2\% agreement.}
\end{column}
\begin{column}{0.5\textwidth}
\centering
\vspace{4pt}
...
\begin{itemize}\setlength\itemsep{7pt}\small
  \item Balance closes to \key{$<$0.25\%} ($\Delta U = -584$\,MJ), and \key{0.80\%} on the finest mesh.
...
\begin{tcolorbox}[hotbox]
\small The average column under-predicts, but the peak-power column (\hot{1268\,\textcelsius}) \key{brackets} the full-core DCC peak (1118\,\textcelsius) from above. Carry \hot{1268\,\textcelsius}, margin $\sim$332\,\textcelsius, as the bounding number; the 793\,\textcelsius\ average column validates the outlet.
\end{tcolorbox}
\end{frame}
...
\begin{itemize}\setlength\itemsep{5pt}\small
  \item The block-scale cooldown peak is non-conservative: a lower bound, not the safety peak.
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
\vspace{6pt}
...
\footnotesize Restore the radial heat path with an effective-core model, moving the RCCS sink to the vessel boundary. That model can be compared directly against the benchmark full-core peak: 1391\,K at 50\,h, or 1237\,K at 74\,h for the ring model.
\end{tcolorbox}
\vspace{7pt}
...
\begin{frame}[plain]
\begin{tikzpicture}[remember picture, overlay]
  \fill[RadAmber] ([xshift=1.3cm,yshift=0.55cm]current page.west) rectangle ([xshift=1.42cm,yshift=2.3cm]current page.west);
  \node[anchor=north west]
        at ([xshift=1.7cm,yshift=2.25cm]current page.west)
    {\begin{minipage}{12.5cm}\color{white}\fontsize{20}{25}\selectfont\bfseries
       A validated 3D peak-fuel prediction, and the reasoning behind it.\end{minipage}};
  \node[anchor=north west, text=RadBlueL, font=\small]
        at ([xshift=1.75cm,yshift=-0.25cm]current page.west)
    {1268\,\textcelsius\ bounding peak \quad $\sim$332\,\textcelsius\ margin \quad outlet 688 vs 687\,\textcelsius\ \quad energy under 0.8\%};
  \node[anchor=west, text=white, font=\footnotesize\bfseries]
        at ([xshift=1.75cm,yshift=-2.15cm]current page.west) {Samay Shah};
  \node[anchor=east, text=RadAmber, font=\footnotesize]
        at ([xshift=-1.4cm,yshift=-2.15cm]current page.east) {Questions welcome};
\end{tikzpicture}
\end{frame}
}
