"""
Generate system/solid/topoSetDict that carves the fuel compacts out of the
solid region into a cellZone named "fuel". The fvOptions heat source is applied to
that cellZone. Geometry comes from fuel_zones.json (written by make_full_block.py).
...
def main():
    ...
    lines = [
        ...
        "// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //",
        "",
        "// Fuel compacts -> cellSet 'fuel' -> cellZone 'fuel' (heat-source zone).",
        "actions",
        "(",
    ]
